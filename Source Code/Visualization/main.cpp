...
// Global variables delarations
string text, pattern;
int Q; // Prime number
Colour text_colour[MAX]; // Text colour array
Colour pattern_colour[MAX]; // Pattern colour array
int display_option=0;
int dfa[MAX][256]; // dfa array

// Functions declarations
void PrintStrings(int, int);
void init_dfa_matrix();
int *BuildLast(string);
bool is_prime(int);
int RandomPrime();
void ColourText(int, Colour);
void ColourPattern(int, Colour);
void ResetTextColour();
void ResetPatternColour();
...
int Rabin_Karp()
{
    ...
    int N = text.size();
    int i, j = 0, k;
    //Q=997;                           // Choose a prime number for the hash functio
    Q=RandomPrime();
    //cout << "Prime: " << Q << endl;
    long p = 0, t = 0, h = 1;
...
int main()
{
    ...
    do
    {
        ...
        for (display_option=0; display_option<2; display_option++)
        {
            switch(option)
                {
                case 1:
